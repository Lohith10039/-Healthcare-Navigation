...
def get_department_recommendation(user_symptoms: str):
    """Alias for semantic search compatibility."""
    chunks = semantic_rag_search(user_symptoms, top_k=1)
    if chunks:
        return {
            "rag_context": chunks[0]["text"],
            "department": chunks[0]["metadata"]["department"],
            "source": chunks[0]["metadata"]["source"]
        }
    return {"rag_context": "No specific guidelines found.", "department": "General Medicine", "source": "System Fallback"}
